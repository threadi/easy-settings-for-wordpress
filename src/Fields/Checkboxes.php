...
class Checkboxes extends Field_Base {
	...
	public function display( array $attr ): void {
		...
		?><div data-depends="<?php echo esc_attr( $this->get_depend() ); ?>">
		<?php
		...
		foreach ( $this->get_options() as $key => $settings ) {
			...
			if ( $this->is_readonly() ) {
				?>
				<input type="hidden" name="<?php echo esc_attr( $setting->get_name() ); ?>[<?php echo esc_attr( $key ); ?>]; ?>" value="<?php echo ( isset( $values[ $key ] ) ? 1 : 0 ); ?>">
				<?php
			}
